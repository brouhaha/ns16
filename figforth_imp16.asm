...
;*               SAN CARLOS, CA 94070              *
;*                                                 *
;*       IMP-16 IMPLEMENTATION BY ERIC SMITH       *
;*                    BASED ON                     *
;*      PACE IMPLEMENTATION BY DAVID KILBRIDGE     *
;*                                                 *
;*           EIS INSTRUCTION SET REQUIRED          *
;***************************************************
;
	.ASECT
	.=	0010
...
;
;***************************************************
;* STATUS FLAG ABBREVIATIONS (SETST/CLRST/SKSTF)   *
;***************************************************
;
LINK	=	15
OV	=	14
...
BACKSP:	.WORD	007F		; BACKSPACE CHARACTER
...
WARN0:	.WORD	1		; ERROR WARNING MODE
...
BLOCKIO	=	07EFF
;
INIT:	LI	0,0		; clear flags
	PUSH    0
	PULLF

	PULL	Y		; get return address

	LI	1,15		; empty the hardware stack
	PULL	0
	AISZ	1,-1
	JMP	.-2
	
	PFLG	IE		; disable interrupts

	PUSH	Y		; not sure why we both push Y and jmp ind
	JMP	(Y)
;
	.BSECT
	.=.+00F6		; POINTER AREA
...
RT1:	SKAZ	0,RMASK		; TEST RIGHT BYTE
	...
	AISZ	X,1		; BUMP COUNT
	CLRST	LINK		; LEFT BYTE OF
	ISZ	0(SP)		;  FOLLOWING WORD
	JMP	NEW1		;   IS NEXT
RNDEL:	SETST	LINK		; REMEMBER WHICH BYTE
LNDEL:	ST	X,2(SP)		; SET FC OFFSET

	SKSTF	LINK		; RESUME AT SAME BYTE
	JMP	.+2
	JMP	RT2
...
USLASH:	.WORD	.+1
	PUSH	IP		; SAVE IP
	LD	0,1(SP)		; HI-ORDER DIVIDEND
	LD	1,2(SP)		; LO-ORDER DIVIDEND
	DIV	0(SP)		; DIVIDE BY DIVISOR
	ST	0,2(SP)		; REMAINDER SECOND
	RCPY	1,0		; MOVE QUOTIENT TO R0
	PULL	IP		; RESTORE IP
...
DPLUS:	.WORD	.+1
	...
	LD	1,3(SP)		; AUGEND LOW-ORDER
	DADD	0(SP)		; ADDEND
	ST	1,3(SP)		; SUM LOW-ORDER
...
TOGGLE:	.WORD	.+1
	...
	SFLG	SEL
	CLRST	LINK		; CONVERT TO
	ROR	X,1		;  WORD ADDRESS
	ST	X,1(SP)		; SAVE FOR LATER
	LD	0,(X)		; GET DATA WORD
	LD	X,0(SP)		; GET BIT PATTERN
	PFLG	SEL
	SKSTF	LINK		; EVEN BYTE ADDRESS
	SHL	X,8		;  MEANS LEFT BYTE
...
CSTORE:	.WORD	.+1
	...
	SKSTF	LINK		;  AND CLEAR
	JMP	.+2
	JMP	CST1

	CAI	0,0		;   APPROPRIATE
	SHL	1,8		;    HALF OF WORD

CST1:	AND	0,(X)		;     TO STORE INTO
...
SUB:	.WORD	.+1
	...
	SUB	0,0(SP)		; SUBTRACT SUBTRAHEND
	JMP	BIN
;
	HEAD	ORD,1,SHORT,'='/256
	.WORD	SUB-2
